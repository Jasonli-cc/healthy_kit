...
class _HealthAppState extends State<HealthApp> {
  ...
  Future<void> addData() async {
    ...
    success &= await Health().writeHealthData(value: 1.925, type: HealthDataType.HEIGHT, startTime: earlier, endTime: now);
    success &= await Health().writeHealthData(value: 90, type: HealthDataType.WEIGHT, startTime: now);
    success &= await Health().writeHealthData(value: 90, type: HealthDataType.HEART_RATE, startTime: earlier, endTime: now);
    success &= await Health().writeHealthData(value: 90, type: HealthDataType.STEPS, startTime: earlier, endTime: now);
    success &= await Health().writeHealthData(value: 200, type: HealthDataType.ACTIVE_ENERGY_BURNED, startTime: earlier, endTime: now);
  ...
  Widget get _contentDataReady => ListView.builder(
      ...
      itemBuilder: (_, index) {
        ...
        if (p.value is WorkoutHealthValue) {
          return ListTile(
            title: Text("${p.typeString}: ${(p.value as WorkoutHealthValue).totalEnergyBurned} ${(p.value as WorkoutHealthValue).totalEnergyBurnedUnit?.name}"),
  ...
  Widget _authorizationNotGranted = const Column(
    children: [
      ...
      const Text('For Google Fit please check your OAUTH2 client ID is correct in Google Developer Console.'),
